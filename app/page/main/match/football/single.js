...
import MatchGrid from '../grid/singleWinerGrid';
...
class Single extends Component {
        ...
        service.matchOddsService(args)
            ...
            .catch(err => {
                if (err.errorcode === 100009) {
                    this.setState({
                        maintain: true, isConnecting: false, errorMessage: err.message,
    ...
    succCallback = () => {
        this.setState({ isOpenSucc: true, activeId: [] });
    }
    modalDidClose = () => {
        this.setState({ isOpenSucc: false, activeId: [] });
    }
    clearActiveId = async (activeId) => {
        await this.setState({ activeId: activeId ? activeId : [] })
    }
    renderItem({ item }) {
        const { activeId, matchEventType } = this.state;
        return (
            <SectionRow {...item} >
                {item.schedules.length > 0 && item.schedules.map((obj, i) =>
                    <View style={styles.boxView} key={i}>
                        <MatchGrid n={obj} key={i} prevActiveId={prevGameId} prevGameId={prevActiveId} activeId={activeId} onSelected={(key) => this.handleSelect(key)} matchTypeId={matchEventType.typeEngName} />
                    </View>
                )}
            </SectionRow>
        );

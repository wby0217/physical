...
class TennisMatchGrid extends Component {
  ...
  render() {
    ...
    const totalScore = homeScore + guestScore;
        ...
        {matchTypeId === 'in_play_now' ?
          <Grid style={{ backgroundColor: '#FFFCE7', marginTop: 5 }}>
            <Row style={{ paddingHorizontal: 10, flex: 1, height: 35, alignItems: 'center' }}>
              <Col style={{ flex: 6 }}>
                <Text>{n.best}|总局数<Text style={{ color: '#FF001F' }}>{homeScore}-{guestScore} ({totalScore})</Text></Text>
              </Col>
        ...
        <Grid style={{ paddingRight: 6, marginTop: 10 }}>
          ...
          <Row style={{ height: 200 }}>
            <Col size={1}>
              ...
              <Row style={[styles.alginCenter, styles.justifyCenter, { flex: 2 }]}><Text style={{ color: '#81907E' }}>球员{'\n'}局数{'\n'}大小</Text></Row>
            </Col>
            <Col size={2.5} style={[styles.bgWhite, { borderLeftColor: '#B6B6B6', borderLeftWidth: StyleSheet.hairlineWidth }]}>
              ...
              {(() => {
                const args = ['iorOuho', 'ouPg', curGameInfo.odds.ouPg.iorOuho].concat([curGameInfo.gameId, `大`, n.parlayMin || [], n.parlayMax || [], n.scheduleId, 'ratioOuho', curGameInfo.odds.ouPg.ratioOuho]);
                const isActive = this.isActive(args);
                return (
                  <Row style={[styles.gridRightTopColor]}>
                    {(curGameInfo.odds.ouPg.iorOuho) ?
                      <TouchableOpacity
                        activeOpacity={1}
                        onPress={() => { // 球员局数大小-主/大
                          onSelected(args);
                      ...
                      >
                        <Text>
                          <Text>
                            <Text style={[styles.grayColor, isActive ? styles.activeTextClass : null]}>大{curGameInfo.odds.ouPg.ratioOuho}</Text>
                            <Text style={[isActive ? styles.activeTextClass : null]}>{` ${curGameInfo.odds.ouPg.iorOuho}`}</Text>
                          </Text>
                        </Text>
                        <View style={{ width: 20, alignItems: 'center' }}>
                          {iorOuho > 0 ? <Icons name="icon-arrow-top" color="#FF6E6E" /> : iorOuho < 0 ? <Icons name="icon-arrow-down" color="#9BFF95" /> : null}
                        </View>
                      </TouchableOpacity>
                      :
                      <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}>
              ...
              {(() => {
                const args = ['iorOuhu', 'ouPg', curGameInfo.odds.ouPg.iorOuhu, curGameInfo.gameId, `小`, n.parlayMin || [], n.parlayMax || [], n.scheduleId, 'ratioOuhu', curGameInfo.odds.ouPg.ratioOuhu];
                const isActive = this.isActive(args);
                return (
                  <Row style={[styles.gridRightTopColor, { borderBottomColor: '#B6B6B6', borderBottomWidth: StyleSheet.hairlineWidth }]}>
                    {(curGameInfo.odds.ouPg.iorOuhu) ?
                      <TouchableOpacity
                        activeOpacity={1}
                        onPress={() => { // 球员局数大小-主/小
                          onSelected(args);
                      ...
                      >
                        <Text>
                          <Text style={[styles.grayColor, isActive ? styles.activeTextClass : null]}>小{curGameInfo.odds.ouPg.ratioOuhu}</Text>
                          <Text style={[isActive ? styles.activeTextClass : null]}>{` ${curGameInfo.odds.ouPg.iorOuhu}`}</Text>
                        </Text>
                        <View style={{ width: 20, alignItems: 'center' }}>
                          {iorOuhu > 0 ? <Icons name="icon-arrow-top" color="#FF6E6E" /> : iorOuhu < 0 ? <Icons name="icon-arrow-down" color="#9BFF95" /> : null}
                        </View>
                      </TouchableOpacity>
                      :
                      <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}>
            ...
            <Col size={2.5} style={styles.bgWhite}>
              ...
              {(() => {
                const args = ['iorOuco', 'ouPg', curGameInfo.odds.ouPg.iorOuco].concat([curGameInfo.gameId, `大`, n.parlayMin || [], n.parlayMax || [], n.scheduleId, 'ratioOuco', curGameInfo.odds.ouPg.ratioOuco]);
                const isActive = this.isActive(args);
                return (
                  <Row style={[styles.gridRightTopColor]}>
                    {(curGameInfo.odds.ouPg.ratioOucu) ?
                      <TouchableOpacity
                        activeOpacity={1}
                        onPress={() => { // 球员局数大小-客/大
                          onSelected(args);
                      ...
                      >
                        <Text>
                          <Text style={[styles.grayColor, isActive ? styles.activeTextClass : null]}>大{curGameInfo.odds.ouPg.ratioOuco}</Text>
                          <Text style={[isActive ? styles.activeTextClass : null]}>{` ${curGameInfo.odds.ouPg.iorOuco}`}</Text>
                        </Text>
                        <View style={{ width: 20, alignItems: 'center' }}>
                          {iorOuco > 0 ? <Icons name="icon-arrow-top" color="#FF6E6E" /> : iorOuco < 0 ? <Icons name="icon-arrow-down" color="#9BFF95" /> : null}
                        </View>
                      </TouchableOpacity>
                      :
                      <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}>
              ...
              {(() => {
                const args = ['iorOucu', 'ouPg', curGameInfo.odds.ouPg.iorOucu, curGameInfo.gameId, `小`, n.parlayMin || [], n.parlayMax || [], n.scheduleId, 'ratioOucu', curGameInfo.odds.ouPg.ratioOucu];
                const isActive = this.isActive(args);
                return (
                  <Row style={[styles.gridRightTopColor, { borderBottomColor: '#B6B6B6', borderBottomWidth: StyleSheet.hairlineWidth }]}>
                    {(curGameInfo.odds.ouPg.iorOucu) ?
                      <TouchableOpacity
                        activeOpacity={1}
                        onPress={() => { // 球员局数大小-客/小
                          onSelected(args);
                      ...
                      >
                        <Text style={isActive ? styles.activeTextClass : null}>
                          <Text style={[styles.grayColor, isActive ? styles.activeTextClass : null]}>小{curGameInfo.odds.ouPg.ratioOucu}</Text>
                          <Text style={isActive ? styles.activeTextClass : null}>{` ${curGameInfo.odds.ouPg.iorOucu}`}</Text>
                        </Text>
                        <View style={{ width: 20, alignItems: 'center' }}>
                          {iorOucu > 0 ? <Icons name="icon-arrow-top" color="#FF6E6E" /> : iorOucu < 0 ? <Icons name="icon-arrow-down" color="#9BFF95" /> : null}
                        </View>
                      </TouchableOpacity>
                      :
                      <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}>

...
import TennisCorrectGrid from './correctGrid';

let prevGameId = List([]);
let prevActiveId = List([]);
const moduleKey = "correct_score";
            ...
            <SectionRow {...item} >
                {item.schedules.length > 0 && item.schedules.map((item, i) =>
                    <View style={styles.boxView} key={i}>
                        <TennisCorrectGrid n={item} key={i} prevActiveId={prevGameId} prevGameId= {prevActiveId} activeId={activeId} onSelected={(key) => this.handleSelect(key)} matchTypeId={matchEventType.typeEngName} /> 
                    </View>
                )}
            </SectionRow>
        )

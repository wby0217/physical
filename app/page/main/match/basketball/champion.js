...
import SectionRow from '../grid/championGrid';

let prevGameId = List([]);
let prevActiveId = List([]);
const moduleKey = "outright";
...
class Champion extends Component {
    ...
    async componentWillReceiveProps(nextProps) {
        ...
        }else if( orderStatus !== nextProps.orderStatus && this.tabIndex === EventTypeIndex) {
            this.pageIndex = 1;
            this.totalPage = 1;
            this.setState({
                data: [],
        ...
        } else if( AllianceId !== nextProps.AllianceId  && this.tabIndex === EventTypeIndex) {
            this.pageIndex = 1;
            this.totalPage = 1;
            this.setState({
                data: [],
    ...
    refreshData = () => {
        ...
        const args = { sportType: navigation.state.params && navigation.state.params.engName, eventType: matchEventType.typeEngName, playTypeGroup: 'outright', pageAll: this.pageIndex, matches: AllianceId || 0 }
        ...
        .then(res => {
            ...
            if(!_.isEmpty(res.data) && res.data.result.length > 0) {
                this.setState({
                    ...
                    // data: _.merge(this.state.data, res.data.result),
                    noData: false,
                    isLoadAll: false,
                    isConnecting: false,
                    maintain: false
    ...
    fetchData(params = {}) {
        ...
        const args = { sportType: navigation.state.params && navigation.state.params.engName, eventType: matchEventType.typeEngName, playTypeGroup: 'outright', page: this.pageIndex, order: orderType, pageAll: params.pageAll || 0, matches: AllianceId || 0 };
    ...
    renderItem({ item }) {
        ...
        return (
          <SectionRow prevActiveId={prevGameId} prevGameId= {prevActiveId} activeId={activeId} rowData={item} onSelected={(key) => this.handleSelect(key)} />
        );
    }
    onRefresh() {
        if(this.isMouted) return;

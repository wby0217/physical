...
import BasketBallGrid from './basketballSingleGrid';
import OrderPop from '../orderPop';

let prevGameId = List([]);
let prevActiveId = List([]);
const isIos = Platform.OS === 'ios';
const moduleKey = "1x2-handicap-ou-oe";
class Single extends Component {
    constructor(props) {
        ...
        this.basketBallRenderItem = this.basketBallRenderItem.bind(this);
    ...
    async componentWillReceiveProps(nextProps) {
        const { matchEventType, navigation, orderStatus, EventTypeIndex, AllianceId, isFocused, timestampMd, selectedBallInfo, isPeriod, isMaster } = this.props;
    ...
    refreshData = () => {
        ...
        const args = { sportType:  navigation.state.params && navigation.state.params.engName, eventType: matchEventType.typeEngName,  playTypeGroup: moduleKey, pageAll: this.pageIndex, matches: AllianceId || 0,  master: isMaster || 'no', period: isPeriod || 'yes' }
    ...
    basketBallRenderItem({ item }) {
        const { activeId, matchEventType } = this.state;
        return (
          <SectionRow {...item} >
            {item.schedules.length > 0 && item.schedules.map((item, i) =>
                <View style={styles.boxView} key={i}>
                     <BasketBallGrid n={item} key={i} prevActiveId={prevGameId} prevGameId= {prevActiveId} activeId={activeId} onSelected={(key) => this.handleSelect(key)} matchTypeId={matchEventType.typeEngName} /> 
                </View>
            )}
          </SectionRow>
        );
            ...
            <KeyboardAvoidingView behavior={isIos ? "padding" : ''}  style={styles.container}>
                {noData ? <Nodata onRefresh={this.onRefresh} />:
                    <FlatList
                        ...
                        renderItem= {this.basketBallRenderItem}
                        ...
                        //     const lineHeight = data[index].schedule.length * 220
                        //     return ({
                        //         length: lineHeight + 40,
                        //         offset: lineHeight * index, index
                        //     })
                        // }}
                    />
                }

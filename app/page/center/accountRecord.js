...
export default class AccountRecord extends Component {
    ...
    fetchData = () => {
        ...
        service.detailRecordsService({ typeId: selectedTypeId, page: this.pageIndex })
        .then(res => {
            ...
            <View>
                ...
                >
                    {item.records && item.records.length > 0 ? item.records.map((obj,n) => {
                        return (
                            <View key={n} style={[{ borderBottomColor: '#E5E5E5', borderBottomWidth: StyleSheet.hairlineWidth, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', height: 65 }, n === (item.records.length-1) ? {borderBottomWidth: 0}: null]}>
                                <View>
    ...
    render() {
        const { refreshing, datalist, filterModalVisible, filterType, selectedTypeId, noDataStatus, detailsModal } = this.state;
        const { navigation } = this.props;
        return (
            <View style={styles.container}>
                ...
                <TopUpRecordDetail open={detailsModal} modalDidClose={this.toggleDetailModal} />
            </View>
        )
    }
}
